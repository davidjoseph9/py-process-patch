...
class PatchManager:
    ...
    def hook(self, pymem_instance: pymem.Pymem, hook_type: HookType, patch: PatchModel,
        ...
        self.VirtualProtect(pymem_instance.process_handle, int(patch.address), len(hook_bytes), pymem.ressources.structure.MEMORY_PROTECTION.PAGE_EXECUTE_READWRITE)
        # if not success:
